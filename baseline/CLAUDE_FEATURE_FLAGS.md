...
1. **`Ku()` (static)** - Calls individual feature functions, builds base object (was `yh()` in v1.19367.0, `sM()` in v1.18286.x). Since v1.19367.0 the result is post-processed by a registry post-processor which stamps `maturity:"beta"` onto supported features listed in `["chatTab","surfaceTogglesPreview","chatCodeExecution"]` (cosmetic, no gating; list unchanged in v1.20186.1).
2. **`Uae` (async merger)** - Spreads `Ku()` and applies the async overrides: returns `{...Ku(),...s}` where `s={louderPenguin,coworkKappa,coworkArtifacts,epitaxyMcpApps,launch,coworkWatchRecord}` (**6 overrides since v1.20186.1**; `launch` = flag `2976814254`, follows server rollout (.jsonc opt-in; the former Patch 3j force was retired 2026-07-13); was 5 overrides — merger `_Be` in v1.19367.0, `Yue` in v1.18286.x). `markTaskComplete` is **no longer an async override** — it was removed in v1.17282.0.
...
| - | *(async overrides in the merger: `louderPenguin`, `coworkKappa`, `coworkArtifacts`, `epitaxyMcpApps`, `launch` (since v1.20186.1), `coworkWatchRecord`)* | See rows 6, 18-19, `epitaxyMcpApps`, 29 | async overrides in merger | `markTaskComplete` removed in v1.17282.0 — no longer an async override |
