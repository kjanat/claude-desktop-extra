#!/usr/bin/env bash
# Guard: docs/claude-desktop-extra.jsonc must equal the template the
...
# it as docs/claude-desktop-extra.jsonc so users can browse the full flag catalog
...
DOCS="${REPO_DIR}/docs/claude-desktop-extra.jsonc"
MODE="${1:-check}"
...
"
GEN="${TMPDIR_GEN}/claude-desktop-extra.jsonc"
if [[ ! -f "${GEN}" ]]; then
...
if ! diff -u "${DOCS}" "${GEN}" >/dev/null 2>&1; then
	echo "::error::docs/claude-desktop-extra.jsonc is out of sync with the template in js/growthbook_overrides.js"
...
echo "[jsonc-sync] docs/claude-desktop-extra.jsonc matches the shipped template"
