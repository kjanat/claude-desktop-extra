...
if (!CHROMIUM) {
	console.log('  SKIP: no chromium on PATH');
	process.exit(3);
}
...
const CHAT_SHELL =
	`<div class="tiles-shell" style="position:absolute;top:0;bottom:0;left:0;width:100%;height:100%"></div>`;
...
function run(html, name, budgetMs) {
	...
	try {
		const dom = execFileSync(CHROMIUM, [
			...
			'--disable-gpu',
			'--no-sandbox',
			'--virtual-time-budget=' + (budgetMs || 2000),
			'--dump-dom',
			'file://' + file,
		], { encoding: 'utf8' });
...
// --- harvester ---------------------------------------------------------------
{
	...
	ok(
		JSON.stringify(r.panes) === '["chat","terminal","diff"]',
		"harvest: every pane root resolves its tileId from the fiber ancestor's memoizedProps",
	);
	ok(
		JSON.stringify(r.buttons) === '[[true,false,true],[true,false,true],[true,false,true]]',
...
}
{
    ...
    ${SINK}({ expand: !!b.expand, collapse: !!b.collapse });`,
		}),
		'harvest-collapse-label',
	);
	ok(
		r.expand === false && r.collapse === true,
...
}
{
	...
	const r = run(
		fixture(['chat', 'diff'], {
			body: `
    var H = window.__cdbTabsHarvest;
    ${SINK}({ keys: Object.keys(H).sort() });`,
		}),
		'harvest-surface',
	);
	ok(
		r.keys.indexOf('onLayoutChange') === -1 && r.keys.indexOf('harvestAll') === -1
	...
	ok(
		JSON.stringify(r.keys) === '["chromeButtons","panes","tileIdOf"]',
...
// suite while still shipping the machinery the user asked to be removed.
{
	...
	const hits = banned.filter(([needle]) => PAGE_SRC.indexOf(needle) !== -1 || HARVEST_SRC_TOP.indexOf(needle) !== -1);
	ok(
		hits.length === 0,
...
// --- enable adopts the split that already exists -----------------------------
{
	...
	ok(
		JSON.stringify(r.tagged) === '["diff","terminal","preview"]',
...
// --- switching: attributes only, zero writes, panes stay mounted --------------
{
	...
	ok(
		r.okSwitch === true && r.mid.active === 'preview',
	...
	ok(
		JSON.stringify(r.mid.hidden) === '["diff","terminal"]',
		'switch: the columns that are not active carry the tag but not the active marker',
	);
	ok(
		r.mid.writes === 0 && r.writes === 0,
		'switch: three tab switches issue ZERO layout writes - onLayoutChange is never called',
	);
	ok(
		JSON.stringify(r.mid.panes) === '[true,true,true]',
		"switch: a hidden column's pane stays in the DOM (this is what preserves its React state)",
	);
	ok(r.taggedUnchanged === true, 'switch: the set of tagged columns does not change - only which one is marked active');
	ok(
		r.finalActive === 'diff' && r.stored.activeId === 'diff',
...
// --- the mechanism's own CSS --------------------------------------------------
{
	...
	ok(r.fillRule === true, 'css: an inner chain element simply fills its parent - it is the only visible child of it');
	ok(
		r.noLeafRules === true,
		'css: NO rule keys off the leaf wrappers any more - data-cdb-col is identity only, because the side region is a nested tree',
	);
	ok(r.heightRule === true, 'css: the shell height-compensation rule is unchanged');
	ok(r.hiddenWidth === 0, 'css: a hidden column really measures 0px wide');
	ok(r.activeWidth > 0 && r.chatWidth > 0, 'css: the active column and the chat column both keep real width');
	ok(
		Math.abs(r.sideFlex - 1) < 1e-9 && Math.abs(parseFloat(r.flexVar) - 1) < 1e-9,
...
}
{
	...
	const r = run(
		withPage(
			['chat', 'diff', 'terminal', 'preview', 'tasks'],
			`
    var P = window.__cdbTabsPage;
    P.setEnabled(true); P.reconcile();
	...
	ok(
		Math.abs(r.fromDom.chatFlex - 1.0481) < 1e-9
	...
	ok(
		Math.abs(r.afterRescale.chatFlex - 2.0) < 1e-9
	...
	ok(
		Math.abs(r.afterUniform.boundary - SHARE) < 1e-9,
...
}
{
	...
	const seed = (entry) =>
		`localStorage.setItem("cdb.panelTabs.v2", JSON.stringify({
	...
	ok(
		Math.abs(r.flex.chatShare - 0.4) < 1e-9 && Math.abs(r.flex.boundary - 0.4) < 1e-9,
		'reload: a persisted share is adopted as-is, so a reload does not jump the boundary',
	);
	ok(
		Math.abs(r.stored.chatShare - 0.4) < 1e-9 && r.stored.activeId === 'terminal',
	...
	ok(
		Math.abs(r.flex.chatShare - captured) < 1e-9,
		'migration: a stored `total` is NOT read as a share - the share is recaptured from the live layout instead',
	);
	ok(
		r.stored.total === undefined && Math.abs(r.stored.chatShare - captured) < 1e-9,
	...
	[
		...
		ok(
			Math.abs(rr.flex.chatShare - captured) < 1e-9,
...
}
{
	...
	ok(
		Math.abs(r.renorm.chatShare - START) < 1e-9 && Math.abs(r.renorm.boundary - START) < 1e-9,
	...
	ok(
		Math.abs(r.afterDrag.chatShare - 2.0 / 3.0) < 1e-9
	...
	ok(Math.abs(r.stored.chatShare - 2.0 / 3.0) < 1e-9, 'drag: and the released position is persisted, so it sticks');
	ok(
		Math.abs(r.dirtyGesture.chatShare - r.baseShare) < 1e-9,
...
}
{
	...
	ok(JSON.stringify(r.tagged) === '[]', 'safety: a column wrapper that also holds the chat pane is never tagged');
	ok(
		r.action === 'no-columns' && r.bar === false,
		'safety: with no taggable column there is no bar at all - the degraded state is the stock split',
	);
	ok(r.writes === 0 && r.warns === 1, 'safety: it warns and touches nothing (no layout write of any kind)');
...
}
{
    ...
    ${SINK}({ action: res.action, tabs: P.state().tabs, sidePanes: P.state().sidePanes,
      ...
      <div class="no-shell-here">${pane('diff')}</div>`,
			},
		),
		'no-shell-wrapper',
	);
	ok(
		r.sidePanes === 1 && JSON.stringify(r.tabs) === '[]',
		'degradation: a side pane with no .tiles-shell yields no column and therefore no tab',
	);
	ok(
		r.action === 'no-columns' && r.bar === false && r.style === false,
...
}
{
	...
	ok(
		JSON.stringify(r.fresh.tabs) === '["diff","terminal"]',
		'fresh column: a column present in the DOM but not yet in the debounced mirror still gets a tab',
	);
	ok(r.fresh.active === 'terminal', 'fresh column: a panel the user just opened becomes the active tab');
	ok(
		JSON.stringify(r.settledTabs) === '["diff","terminal"]' && r.settledActive === 'terminal',
...
}
{
	...
	ok(
		r.expanded.active === 'diff' && JSON.stringify(r.expanded.hidden) === '["terminal","preview"]',
		'expanded: the expanded tile stays active and the rest stay hidden',
	);
	ok(
		JSON.stringify(r.afterMirrorDrop) === '["diff","terminal"]',
...
}
{
	...
	ok(
		r.disabled.closed === false && r.empty.closed === false && r.unknown.closed === false,
		'close: refused while disabled, with no tileId, and for a tileId that has no column',
	);
	ok(JSON.stringify(r.close) === '[]' && r.writes === 0, 'close: a refusal clicks nothing and writes nothing');
}
...
// --- the bar ------------------------------------------------------------------
{
	...
	ok(r.role === 'tablist' && r.label === 'Side panels', 'bar: role="tablist" semantics are unchanged');
	ok(
		JSON.stringify(r.tabLabels) === '["Diff","Terminal"]',
		'bar: one chip per side tile, labelled from the tileId->kind map',
	);
	ok(JSON.stringify(r.selected) === '["true","false"]', 'bar: aria-selected tracks the active tab');
	ok(
		JSON.stringify(r.closeLabels) === '["Close Diff","Close Terminal"]',
		'bar: every chip carries its own ✕ with a per-tab accessible name',
	);
	ok(r.expandLabel === 'Expand', 'bar: the ⤢ is rendered with its state-tracking accessible name');
	ok(
		JSON.stringify(r.order) === '["cdb-tabs-strip","cdb-tabs-sep","cdb-tabs-expand"]',
	...
	ok(
		r.hostIsActiveColumn === true && r.nextIsShell === true,
...
}
{
    ...
    ${SINK}(Object.assign(out, { writes: window.__writes() }));`,
		),
		'bar-delegated-clicks',
	);
	ok(
		r.switched[0] === 'terminal' && JSON.stringify(r.switched[1]) === '["false","true"]',
		'bar: clicking a chip switches the column and moves aria-selected',
	);
	ok(
		JSON.stringify(r.closed) === '[["diff","click"]]',
...
}
{
	...
	ok(
		r.action === 'no-columns' && JSON.stringify(r.tabs) === '[]' && r.bar === null,
		'chat-only: no side column means no tabs and no bar',
	);
	ok(JSON.stringify(r.tagged) === '[]' && r.writes === 0, 'chat-only: nothing is tagged and nothing is written');
...
// --- the ⤢ -------------------------------------------------------------------
{
	...
	ok(r.beforeLabel === 'Expand', '⤢: the accessible name is "Expand" while the active panel is collapsed');
	ok(
		JSON.stringify(r.afterExpandClick) === '[["diff","Expand"]]',
		"⤢: it clicks upstream's own Expand button on the active pane - a native click, not a stale harvested closure",
	);
	ok(
		r.expandedLabel === 'Collapse',
		'⤢: once upstream\'s control reads "Collapse" so does ours - the name always matches what the click will do',
	);
	ok(
		JSON.stringify(r.calls) === '[["diff","Expand"],["diff","Collapse"]]',
...
// defect, asserted structurally.
{
	...
	ok(
		r.diff.struct.rowFound === true && r.diff.struct.chain.length === 2
	...
	ok(
		ch(r.diff, 0).side === true && ch(r.diff, 0).fill === false
	...
	ok(
		ch(r.preview, 0).side === true && ch(r.preview, 0).col === 'preview',
		'nested: for a row-level tile the leaf IS the row-level element, so it carries the side flex itself',
	);
	ok(
		!!r.diff.struct.row && r.diff.struct.row.row === true && r.diff.struct.row.side === false,
	...
	ok(
		tabsOf.every((s) =>
			s.boxes.every((b) =>
				b.w === 0 || b.visiblePanes > 0 || b.handle
				|| b.col === 'chat'
			)
		),
	...
	ok(
		r.preview.boxes.filter((b) => b.stack)[0].w === 0
	...
	ok(
		r.diff.boxes.filter((b) => b.col === 'preview')[0].hide === true
	...
	ok(
		r.diff.chatW === r.terminal.chatW && r.diff.chatW === r.preview.chatW && r.diff.chatW > 0,
	...
	ok(
		tabsOf.every((s) => Math.abs(s.sideFlex - r.diff.sideFlex) < 1e-9)
	...
	ok(
		r.diff.stackKids.filter((k) => k.handle).every((k) => k.hide === true)
	...
	ok(
		r.diff.barY === r.terminal.barY && r.diff.barY === r.preview.barY,
		"nested: the bar sits at the same y on every tab - the stack's own handle no longer pushes it down 12px",
	);
	ok(
		r.diff.shellH === r.terminal.shellH && r.diff.shellH === r.preview.shellH
			&& r.diff.shellH > 0,
	...
	ok(
		tabsOf.every((s) =>
			s.boxes.filter((b) => b.col === 'chat')[0].hide === false
...
// the UNION of the mirror's side tiles and the resolvable columns.
{
	...
	ok(
		JSON.stringify(r.before.tabs) === '["diff","terminal","preview"]'
	...
	ok(
		JSON.stringify(r.columns) === '[{"tileId":"terminal","mounted":true}]',
		'union: and it is honest about it - only one column actually resolves',
	);
	ok(
		JSON.stringify(r.chips) === '[["diff",false],["terminal",true],["preview",false]]',
		'union: every tab renders, and the ✕ is OMITTED on the two whose panes are unmounted',
	);
	ok(
		r.active === 'terminal' && r.activeCount === 1 && r.terminalWidth > 0,
		'union: a tab with no column is never treated as the active column - the expanded one stays active and visible',
	);
	ok(r.bar === true && r.barInTerminal === true, 'union: the bar is still there, hosted in the one column that exists');
	ok(
		r.closeRefused.closed === false,
...
}
{
	...
	ok(
		JSON.stringify(r.notExpanded.tabs) === '["diff"]'
	...
	ok(r.notExpanded.expandedTileId === null, 'ghost tab: and the state agrees nothing is expanded');
	ok(
		JSON.stringify(r.expandedTabs) === '["diff","terminal"]'
...
// collapse -> wait for the remount -> re-tag -> expand again, bounded at 1200ms.
{
	...
	ok(
		JSON.stringify(r.hidden) === '["diff"]' && JSON.stringify(r.tabs) === '["diff","terminal"]',
...
}
{
	...
	ok(
		JSON.stringify(r.later.expandCalls) === '[]' && r.later.writes === 0
			&& r.later.warns === 0,
	...
	ok(
		r.later.active === 'terminal' && JSON.stringify(r.later.panes) === '[true,true,true]'
...
}
{
    ...
    setTimeout(function () {
      ${SINK}({ calls: window.__calls.expand.slice(), active: window.__activeCol(),
		...
		'sticky-expand-no-remount',
		2500,
	);
	ok(
		JSON.stringify(r.calls) === '[["diff","Collapse"]]',
		'sticky timeout: the collapse happened and NO expand was ever fired - never expanded onto a pane that is not there',
	);
	ok(
		r.active === 'terminal' && JSON.stringify(r.tabs) === '["diff","terminal"]' && r.bar === true,
		'sticky timeout: it ends collapsed but CORRECT - the chosen tab is active, the strip and the bar are still there',
	);
	ok(r.warns === 1, 'sticky timeout: exactly one warn - the budget expiring is reported once, not once per probe');
	ok(
		r.label === 'Expand',
...
}
{
    ...
    setTimeout(function () {
      ${SINK}({ calls: window.__calls.expand.slice(), active: window.__activeCol(),
		...
		'sticky-expand-no-control',
		2500,
	);
	ok(JSON.stringify(r.calls) === '[]', 'sticky, no control: with neither control resolvable nothing is clicked at all');
	ok(
		r.active === 'terminal' && JSON.stringify(r.tabs) === '["diff","terminal"]' && r.bar === true,
		'sticky, no control: the switch still lands - chosen tab active, strip and bar present',
	);
	ok(
		r.warns === 0 && r.label === 'Expand',
...
}
{
	...
	ok(r.label === 'Collapse' && r.writes === 0, 'supersede: the ⤢ label matches the end state, with zero layout writes');
}
{
	const r = run(
		withPage(
...
// --- upstream's redundant chrome controls are hidden while our bar is present --
{
	...
	ok(
		r.withBar.expand === 'none' && r.withBar.close === 'none',
	...
	ok(
		JSON.stringify(r.clickableWhileHidden) === '[["diff","click"]]',
		'chrome: a display:none control is still clickable - which is exactly what our ✕ and ⤢ rely on',
	);
	ok(
		r.withoutBar.collapse !== 'none' && r.withoutBar.close !== 'none',
...
// --- disable / stop -----------------------------------------------------------
{
    ...
    ${SINK}({ on: on, off: off, reActive: window.__activeCol(),
      reTagged: window.__taggedCols(), writes: window.__writes() });`,
		),
		'disable',
	);
	ok(
		r.on.tagged.length === 3 && r.on.active === 'preview' && r.on.hiddenWidth === 0,
		'disable: the feature really was on and hiding columns first',
	);
	ok(
		JSON.stringify(r.off.tagged) === '[]' && r.off.activeAttrs === 0,
		'disable: every data-cdb-col / data-cdb-col-active attribute is removed',
	);
	ok(
		r.off.style === false && r.off.flexVar === '',
	...
	ok(
		r.off.widths[0] > 0 && r.off.widths[1] > 0 && r.off.widths[2] > 0 && r.off.widths[3] > 0,
		'disable: all four columns are visible again - nothing is stranded',
	);
	ok(
		r.reActive === 'preview' && r.reTagged.length === 3,
...
}
{
    ...
    ${SINK}({ on: on, bar: P.barEl() !== null, tagged: window.__taggedCols(),
      ...
      observed: P._observedRoot(), writes: window.__writes() });`,
		),
		'stop-teardown',
	);
	ok(r.on.bar === true && r.on.tagged === 2, 'stop: the module was live first');
	ok(
		r.bar === false && JSON.stringify(r.tagged) === '[]' && r.style === false,
...
//   M1  a side wrapper whose pane had not arrived yet passed as the chat column
{
	...
	// --- H2: a decoy as the row's FIRST child (overlay, portal mount, drag ghost).
	{
      ...
      ${SINK}({ chat: chatState(), marks: marks(),
        ...
        warns: window.__cdbTabsWarnCount || 0 });`,
			...
			'chat-invariant-decoy',
		);
		ok(
			r.chat.visible === true && r.chat.width > 1,
			'chat invariant / decoy first row child: the chat column is still VISIBLE with real width - reproduced at 0px before the fix',
		);
		ok(r.chat.hasHide === false, 'chat invariant / decoy: and it never carries data-cdb-hide');
		ok(
			r.decoyIsChat === false,
	...
	//     furniture and no row child looks like chat.
	{
      ...
      ${SINK}({ chat: chatState(), marks: marks(),
		...
		ok(
			r.chat.hasHide === false && r.marks.hideOn.length === 0,
			'chat invariant / row unresolvable: NOTHING is hidden at all - no row means we cannot say what belongs to the side region',
		);
		ok(r.marks.armedCount === 0, 'chat invariant / row unresolvable: and the absence rule is not armed');
		ok(
			r.warns === 1,
...
// is switched on. Without that assertion the scenario proves nothing.
{
	...
	// --- A: virgin row, in-flow decoy FIRST. The chat pick must not go by position.
	{
      ...
      ${SINK}(s);`,
			...
			'h2-virgin-inflow-decoy',
		);
		ok(
			r.preDecoyWidth > 1,
			'H2 virgin row: the planted decoy really is a wide in-flow box BEFORE the feature arms (' + r.preDecoyWidth
				+ 'px) - the earlier probes all measured 0 here and so tested nothing',
		);
		ok(r.preChatWidth > 1, 'H2 virgin row: and the real chat column has real width before arming too');
		ok(
			r.decoy.isChat === false && r.chatAttrOn === 'chat',
	...
	// --- B: virgin row, decoy that ALSO owns an empty shell. Two candidates -> refuse.
	{
      ...
      ${SINK}(s);`,
			...
			'h2-virgin-ambiguous-decoy',
		);
		ok(r.preDecoyWidth > 1, 'H2 ambiguity: the shell-owning decoy is a real wide box before arming');
		ok(
			r.chatAttrCount === 0,
		...
		ok(
			r.chat.visible === true && r.decoy.visible === true,
			'H2 ambiguity: both the real chat column and the decoy stay visible - we blank neither when we cannot tell them apart',
		);
		ok(r.diffVisible === true, "H2 ambiguity: upstream's own split is left intact, so every panel is still reachable");
		ok(
			r.warns === 1,
		...
		ok(
			r.upstreamControlsVisible === true,
	...
	//     and the decoy must survive: absence-keying may only reach tile furniture.
	{
      ...
      ${SINK}(s);`,
		...
		ok(
			r.immediate.chatWidth > 1 && r.chat.visible === true,
	...
	//     kinds of tile furniture have to be hidden from their very first layout.
	{
      ...
      ${SINK}({ branchDisplay: getComputedStyle(branch).display,
		...
		ok(r.chatWidth > 1, 'H2 transient: with the chat column untouched throughout');
	}
	...
	//     ABSOLUTE, so it is not a candidate at all and there is no ambiguity to refuse.
	{
      ...
      var after = { chat: Math.round(window.__col("chat").getBoundingClientRect().width),
        ...
        warns: window.__cdbTabsWarnCount || 0 };
      ${SINK}({ before: before, mid: mid, after: after });`,
			),
			'new1-pending-wrapper',
		);
		ok(
			r.before.armed > 0 && r.before.chat > 1,
			'NEW-1: the row was armed with a real chat width before the pending wrapper arrived',
		);
		ok(
			r.mid.pendingIsCandidate === false,
	...
	//     chat 402 -> 248px with two side branches visible and ZERO warns. Silently wrong.
	{
      ...
      ${SINK}({ before: before,
        ...
        warns: window.__cdbTabsWarnCount || 0 });`,
			...
			'new2-untagged-paneless-leaf',
		);
		ok(
			r.before.rowIsHost === true && r.before.chat > 1,
			'NEW-2: the row resolved to the real tiles row before the new leaf arrived',
		);
		ok(
			r.leafIsChat === false && r.chatAttrOn === 'chat',
		...
		ok(
			r.rowIsHost === true && r.rowIsStack === false,
		...
		ok(r.chat === r.before.chat, 'NEW-2: chat does not move - review measured 402 -> 248px here');
		ok(r.visibleSideBranches === 1, 'NEW-2: exactly one side branch is visible - diff and preview both painted before');
		ok(
			r.sticky === true,
	...
	//     has to be released and re-decided.
	{
      ...
      ${SINK}({ held: held, afterInvalidate: afterInvalidate,
		...
		ok(r.held === true, 'sticky: the pick is held once identified');
		ok(
			r.afterInvalidate.sticky === false,
			'sticky: the hold is RELEASED the moment the held element stops being a valid chat column - it is not a permanent lock-in',
		);
		ok(r.afterInvalidate.chatVisible === true, 'sticky: and the chat column stays visible while the pick is unsettled');
		ok(
			r.recovered.sticky === true && r.recovered.chatAttrOn === 'chat',
			'sticky: it re-identifies and re-holds afterwards, so a transient never leaves the feature inert',
		);
		ok(r.recovered.armed > 0 && r.recovered.hide > 0, 'sticky: with arming and hiding both working again');
	}
}
...
// --- M2: a switch from the HOLD path must not destroy the stored share ----------
{
    ...
    ${SINK}({ action: res.action, afterHold: afterHold, afterSwitch: afterSwitch });`,
			{ extraSetup: seed, mirror: ['diff', 'terminal'] },
		),
		'hold-preserves-share',
	);
	ok(r.action === 'hold', 'hold share: the fixture really reaches the hold path');
	ok(
		r.afterSwitch.chatShare === 0.7,
	...
	ok(r.afterSwitch.activeId === 'terminal', 'hold share: while the active tab is still recorded');
}
// --- M5: total anchor loss warns instead of dying silently ----------------------
{
	...
	ok(
		r.action === 'no-columns' && r.bar === false && JSON.stringify(r.tabs) === '[]',
		'anchor rot: with both pane anchors gone there is no bar - the layout is left exactly as upstream ships it [got action='
			+ r.action + ' bar=' + r.bar + ' tabs=' + JSON.stringify(r.tabs) + ' sidePanes=' + r.sidePanes + ']',
	);
	ok(
		r.warns === 1,
...
}
{
	...
	ok(
		JSON.stringify(r.tabs) === '[]' && r.bar === false && r.warns === 0,
...
// --- M4: the bar guard must not drive computeView() on transcript churn ---------
{
    ...
    setTimeout(function () {
      ...
      Promise.resolve().then(function () {
		...
		'bar-guard-not-driven-by-churn',
		2500,
	);
	ok(r.noBar === true, 'bar guard: the fixture is in the no-bar state, where the cheap early-out does not apply');
	ok(
		r.churn.computeViews === 0,
...
}
{
    ...
    setTimeout(function () {
      ...
      Promise.resolve().then(function () {
		...
		'bar-guard-still-restores',
		2500,
	);
	ok(
		r.had === true && r.gone === true && r.back === true,
...
// --- L1: a blocked localStorage must not be retried on every pass ---------------
{
	...
	ok(r.warns === 1, 'blocked storage: and it warns exactly once');
}
...
// reconciles SYNCHRONOUSLY (a MutationObserver callback runs before layout/paint).
{
    ...
    setTimeout(function () {
      ...
      Promise.resolve().then(function () {
		...
		'transient-open-close',
		4000,
	);
	// --- OPEN
	ok(r.steady.vis === 1 && r.steady.chat > 0, 'transient: steady state is one visible side branch');
	ok(
		r.insertSync.vis === 1 && r.insertSync.chat === r.steady.chat,
	...
	ok(
		r.newPaneVisibleMicro === true && r.activeMicro === 'tasks',
	...
	ok(
		r.insertMicro.vis === 1 && r.insertMicro.chat === r.steady.chat,
		'transient/open: and at that point it is still one visible branch at the unchanged chat width',
	);
	ok(
		r.settledAfterInsert.vis === 1 && r.settledAfterInsert.chat === r.steady.chat,
...
}
{
	...
	ok(
		r.armed === true && r.chatExempt === true,
...
}
{
	...
	ok(
		r.addRendered === false && r.menuInDom === false,
	...
	ok(r.expandRendered === true, 'deleted: the expand control stays - only the + went');
	ok(
		JSON.stringify(r.htmlAttrsTouched) === '[]',
	...
	ok(r.probeStyleInjected === false, "deleted: the probe's hide stylesheet is never injected");
	ok(
		r.hooks === 0,
...
}
{
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
		...
		'upstream-open-without-add',
		3000,
	);
	ok(
		JSON.stringify(r.before) === '["diff"]' && JSON.stringify(r.tabs) === '["diff","terminal"]',
		'no +: a panel upstream opens by itself still becomes a tab',
	);
	ok(
		r.rightAway.active === 'terminal',
		'no +: and it is made active by the microtask after the insertion - before paint, not a 120ms debounce later',
	);
	ok(r.active === 'terminal' && r.activeVisible === true, 'no +: it stays active and visible');
	ok(r.writes === 0 && r.warns === 0, 'no +: adopting an upstream-opened panel writes nothing and warns nothing');
}
...
// bar's visible client box.
{
	...
	TAB_COUNTS.forEach((n) => {
      ...
      ${SINK}({ tabs: P.state().tabs.length,
		...
		ok(
			r.expand.insideBar === true && r.expand.hitsSelf === true,
			n + " tabs: the ⤢ is inside the bar's visible box AND hit-tests to itself at its centre",
		);
		ok(r.bar.h === 34, n + " tabs: the bar is still exactly 34px tall - the chips' scrollbar never grows it");
		ok(
			r.bar.scrollW <= r.bar.clientW + 1,
...
}
{
	...
	const r = run(
		withPage(
			['chat', 'diff', 'terminal', 'preview', 'artifact', 'browser', 'tasks', 'pr', 'simulator', 'transcript', 'file'],
			`
    var P = window.__cdbTabsPage;
    P.setEnabled(true); P.reconcile(); P.renderBar();
    ...
    ${SINK}(out);`,
		...
		'bar-active-chip-revealed',
	);
	ok(r.overflowing === true, 'chip strip: with 10 tabs the chips really do overflow their scroller');
	ok(
		r.atFirst.visible === true && r.atFirst.scrollLeft === 0,
		'chip strip: the initially-active chip is the first one and needs no scrolling',
	);
	ok(
		r.afterCtrl9.label === 'transcript' && r.afterCtrl9.visible === true
			&& r.afterCtrl9.scrollLeft > 0,
		'chip strip: Ctrl+9 selects the 9th tab AND scrolls it into view - the selection is never invisible',
	);
	ok(
		r.afterCtrl1.label === 'diff' && r.afterCtrl1.visible === true
	...
	ok(
		r.afterManualThenSwitch.label === 'transcript' && r.afterManualThenSwitch.visible === true,
    ...
    }`;
{
	...
	ok(r.second[0] === true && r.second[1] === 'terminal', 'keys: Ctrl+2 activates the second tab and preventDefaults');
	ok(r.third[0] === true && r.third[1] === 'preview', 'keys: Ctrl+3 activates the third');
	ok(
		r.noCtrl[0] === false && r.noCtrl[1] === 'preview'
			&& r.alt[0] === false && r.alt[1] === 'preview',
		'keys: a bare digit and Ctrl+Alt+digit are both left alone',
	);
	ok(r.outOfRange[0] === false && r.outOfRange[1] === 'preview', 'keys: Ctrl+9 with three tabs is not handled');
	ok(r.tabChord[0] === false, 'keys: Ctrl+Tab is never claimed - the page already uses it');
	ok(r.repeat[0] === false && r.repeat[1] === 'preview', "keys: a held key's auto-repeat is ignored");
...
}
{
	...
	ok(
		r.azerty[0] === true && r.azerty[1] === 'terminal',
		'keys: ev.code is authoritative, so Ctrl+2 works on AZERTY where ev.key is "&"',
	);
	ok(
		r.numlockOff[0] === false && r.numlockOff[1] === 'terminal',
		'keys: a numpad key with NumLock OFF (key:"End") is NOT claimed',
	);
	ok(r.numlockOn[0] === true && r.numlockOn[1] === 'diff', 'keys: the same numpad key with NumLock ON is');
	ok(
		r.codeFallback[0] === true && r.codeFallback[1] === 'preview',
...
}
{
	...
	ok(
		r.xterm[0] === false && r.xterm[1] === 'diff',
		'keys: the shortcut stands down inside the xterm surface - a shell legitimately wants Ctrl+1',
	);
	ok(
		r.input[0] === false && r.editable[0] === false && r.ccdTerminal[0] === false,
		'keys: and inside an input, a contenteditable, and [data-ccd-terminal]',
	);
	ok(
		r.activeElementAfterBlur === true && r.afterBlur[0] === true && r.afterBlur[1] === 'terminal',
...
}
{
    ...
    ${SINK}({ handled: handled, active: activeWhileOn, callsAfterOne: callsAfterOne,
      disabledHandled: disabledHandled, tagged: window.__taggedCols() });`,
		),
		'keys-install-idempotent',
	);
	ok(r.handled === true && r.active === 'terminal', 'keys: the shortcut works after three installShortcuts() calls');
	ok(
		r.callsAfterOne === 1,
	...
	ok(
		r.disabledHandled === false && JSON.stringify(r.tagged) === '[]',
...
// --- live loop --------------------------------------------------------------
{
	...
	ok(
		r.afterEnable && r.afterEnable.bar === true && r.afterEnable.active === 'diff',
		'loop: the pref poll enables the feature without a restart and the bar appears',
	);
	ok(
		JSON.stringify(r.afterOpen.tabs) === '["diff","terminal"]' && r.afterOpen.active === 'terminal',
		"loop: a panel opened through upstream's own UI is picked up by the observer and becomes active",
	);
	ok(
		JSON.stringify(r.afterOpen.hidden) === '["diff"]',
	...
	ok(
		r.observed === true && r.observedIsRow === true,
...
}
{
	...
	ok(
		r.bar === true && r.observedOnce === true,
		'loop: start() is idempotent - three calls install one observer pair and one sweep',
	);
	ok(
		r.afterStop.bar === false && r.afterStop.observed === null && r.doubleStopOk === true,
...
// here - the tokens and the structure can.
{
    ...
    ${SINK}({
      ...
      barNoBorder: barRule.indexOf("border-bottom") === -1 });`,
		),
		'bar-surface-tokens',
	);
	ok(
		r.barUsesBg100 === true && r.barNoLongerBg200 === true,
	...
	ok(r.focusVisibleUntouched === true, 'surface: the stylesheet still says nothing about :focus-visible');
	ok(
		r.barNoBorder === true,
...
// --- labels ------------------------------------------------------------------
{
    ...
    ${SINK}({ known: [P.labelFor("diff"), P.labelFor("terminal"), P.labelFor("preview"),
      ...
      empty: P.labelFor(null) });`,
		),
		'labels',
	);
	ok(
		JSON.stringify(r.known) === '["Diff","Terminal","Browser","Files","PR","Tasks"]',
	...
	ok(r.suffixed === 'Diff 2', 'labels: an instance-suffixed tile id gets an ordinal rather than showing the raw id');
	ok(r.unknown === 'Weirdnew' && r.empty === '', 'labels: an unknown id falls back to itself, capitalised');
}
...
// --- degradation: unreadable mirror ------------------------------------------
{
	...
	ok(
		r.action === 'applied' && JSON.stringify(r.tabs) === '["diff","terminal"]',
...
}
{
	...
	ok(
		r.switched === true && r.active === 'terminal',
		'degradation: a switch still lands when localStorage is blocked - the attribute toggle does not depend on it',
	);
	ok(
		JSON.stringify(r.tabs) === '["diff","terminal"]' && r.bar === true,
		"degradation: no tab is lost - upstream's layout is the record of what is open",
	);
	ok(
		r.warns === 1,
		'degradation: the failed persist is warned once - exactly once, so a warn-per-poll regression cannot pass',
	);
}
