...
/*__CDB_PANEL_TABS__*/ (function() {
	...
	function log(m) {
		(globalThis.__cdbDiag || console.log)('[panel-tabs] ' + m);
	}
	...
	function pathFor(name) {
		try {
			(globalThis.__cdbCfgMigrate || function() {})();
		} catch (e) {}
		var d = userDir();
	...
	function writePref(value) {
		...
		if (raw !== null) {
			...
			try {
				...
				return {
					ok: false,
					error: p + ' is not valid JSON (' + e2.message
			...
			if (stripped !== raw) {
				...
				try {
					_fs.writeFileSync(p + '.cdb-bak', raw, { flag: 'wx' });
				} catch (e3) {}
			}
		}
	...
	function originAllowed(rawUrl) {
		var origin;
		try {
			origin = new _URL(String(rawUrl)).origin;
		} catch (e) {
			return false;
		}
	...
	_ipc.handle('cdb-tabs:pref-set', function(ev, enabled) {
		...
		if (disk.source === 'jsonc-locked') {
			return {
				ok: false,
				error: PREF_KEY + ' is set in ' + JSONC_NAME
	...
	function injectHost(rawUrl) {
		var host;
		try {
			host = new _URL(String(rawUrl)).hostname;
		} catch (e) {
			return false;
		}
	...
	setTimeout(function() {
		var disk = readPrefFromDisk();
		log(
			'installed (main); pref ' + PREF_KEY + '=' + disk.value + ' (source: ' + disk.source + ')'
